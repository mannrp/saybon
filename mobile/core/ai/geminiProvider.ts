// Saybon v2 — Google Gemini AI Provider Implementation
...
// Platform-independent local unique ID generator ( Herms-friendly )
  ...
  private parseVocabularyResponse(text: string, params: GenerationParams): Exercise[] {
    ...
    try {
      ...
      return vocabulary.map((item: any) => ({
        ...
        acceptableAnswers: [],
        hint: `Think about ${item.topic || 'vocabulary'}`,
  ...
  private parseConjugationResponse(text: string, params: GenerationParams): Exercise[] {
    ...
    try {
      ...
      return conjugations.map((item: any) => ({
        ...
        acceptableAnswers: [],
        hint: `Think about ${item.topic || 'conjugation'}`,
